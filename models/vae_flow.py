...
from .flow import *
from .PointDiffusionTransformer import PointDiffusionTransformer
import importlib
import sys
...
class PDT(Module):
    ...
    def __init__(self, args):
        ...
        self.diffusion = DiffusionPoint(
            net = PointDiffusionTransformer(num_class=args.num_class),
    ...
    def get_loss(self, x, kl_weight, label, writer=None, it=None):
        """
        Args:
        ...
        neg_elbo = self.diffusion.get_loss(x, z, label)
    ...
    def AM_sample(self, w, tar_cls, AM_lr, num_points, classifier, cls_noise_guide, flexibility, truncate_std=None):
        batch_size, _ = w.size()
        ...
        samples = self.diffusion.AM_sample(num_points, context=w, tar_cls=tar_cls, AM_lr=AM_lr, classifier=classifier, cls_noise_guide=cls_noise_guide, flexibility=flexibility)
        return samples
    
    
    def sample(self, w, label, num_points, flexibility, truncate_std=None):
        batch_size, _ = w.size()
        if truncate_std is not None:
            w = truncated_normal_(w, mean=0, std=1, trunc_std=truncate_std)
        # Reverse: z <- w.
        z = self.flow(w, reverse=True).view(batch_size, -1)
        samples = self.diffusion.sample(num_points, context=w, label=label, flexibility=flexibility)
        return samples
