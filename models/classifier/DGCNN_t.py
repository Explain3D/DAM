...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul  7 18:02:12 2023
...
class DGCNN_cls(nn.Module):
    def __init__(self, k=20,  emb_dims=1024, dropout=0.5, output_channels=40, device='cpu'):
        ...
        self.conv5 = nn.Sequential(nn.Conv1d(512, emb_dims, kernel_size=1, bias=False),
                                   self.bn5,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.linear1 = nn.Linear(emb_dims*2 + 8, 512, bias=False)
    ...
    def forward(self, x, t):
        ...
        self.feature_compare1 = x.clone().detach()
        
        x = self.conv5(x)                       # (batch_size, 64+64+128+256, num_points) -> (batch_size, emb_dims, num_points)
        x1 = F.adaptive_max_pool1d(x, 1).view(batch_size, -1)           # (batch_size, emb_dims, num_points) -> (batch_size, emb_dims)
        x2 = F.adaptive_avg_pool1d(x, 1).view(batch_size, -1)           # (batch_size, emb_dims, num_points) -> (batch_size, emb_dims)
        x = torch.cat((x1, x2, t), 1)              # (batch_size, emb_dims*2)
        x = F.leaky_relu(self.bn6(self.linear1(x)), negative_slope=0.2) # (batch_size, emb_dims*2) -> (batch_size, 512)
        x = self.dp1(x)
        x = F.leaky_relu(self.bn7(self.linear2(x)), negative_slope=0.2) # (batch_size, 512) -> (batch_size, 256)
        x = self.dp2(x)
        self.feature_compare2 = x.clone().detach()
...
if __name__ == "__main__":
    ...
    label = torch.randint(40, (5, 1))
    t = torch.randn([5,8])
    net = DGCNN_cls(k=5, emb_dims=1024, dropout=0.5, output_channels=40)
    
    res = net(x,t)
    loss = cal_loss(res,label)
    print(label)
    print(res.shape)
    print(loss)